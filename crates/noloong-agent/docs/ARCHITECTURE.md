...
Goal 和 Automation 也在这个 host/interaction 层实现，而不进入 providerless core。Goal 是 session-scoped lifecycle record：每个 session 最多一个 active goal，`TurnCompleted` listener 只在 goal pursuing 时注入一条 `goal_audit` steering observation；模型必须通过 `agent.goal.update` 显式写回状态，工具 provider 始终在 runtime 中可执行，但每次模型请求前会根据当前 active goal 动态决定是否暴露给模型。Automation 是 trigger-agnostic record：MVP 只实现 `time` trigger，但 delivery path 不依赖 cron 语义；现有 busy session 走 steering，idle/completed session 走 prompt，pure automation session 通过 metadata 和 system prompt addition 标记身份。
...
`AgentManifest::default()` 默认启用 `BuiltInToolName::ALL` 中的普通 Rust built-in tools，包括 `host.exec.*`、`agent.goal.update` 和 `agent.manifest.propose_patch`。关闭默认工具使用 `disable_tool` manifest patch；`enable_tool` 对默认已启用工具是幂等恢复操作。文件编辑工具不属于 `BuiltInToolName::ALL`：runtime build 阶段会先移除 `write_file` 和 `apply_patch`，再根据 `fileEditToolPolicy` 选择 exactly one，或禁用两者。
