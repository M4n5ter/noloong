...
即使在 `noloong telegram` 单进程模式下，Telegram bridge 也通过 loopback WebSocket JSON-RPC 连接 host 的 `InteractionControlHandler`。这样单进程产品形态和分进程部署共享同一套协议路径，不给 Telegram 开后门，也确保第三方语言 bridge 能复用同样的 contract。
