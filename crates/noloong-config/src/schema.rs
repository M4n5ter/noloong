...
fn completion_rank(typed: &str, label: &str) -> usize {
    ...
    2 + levenshtein_distance(&typed, &label).min(8)
}
...
mod tests {
    use super::{
        ProfileConfigSchemaCompletionKind, ProfileConfigSchemaIndex,
        ProfileConfigSchemaPathSegment, parse_validated_profile_config_text,
        profile_config_schema_json, profile_config_schema_value, validate_profile_config_schema,
    };
