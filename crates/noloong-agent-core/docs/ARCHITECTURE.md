...
1. 用 `TokenEstimator` 估算当前 `AgentState.messages` 的输入 token 数。
2. 当估算值达到 `input_limit_tokens * trigger_ratio` 时，从最新消息向前保留约 `keep_recent_tokens` 的上下文。
...
5. 调用 `ContextCompactor` 生成新的结构化 summary 或 replacement history，摘要预算来自 `summary_budget_tokens`。
