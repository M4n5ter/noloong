...
Composer 是阅读面的可见输入焦点。默认是稳定横向胶囊，单行 text field 语义，prompt 变长时不继续撑高。一旦内容变成长文本、出现换行或发生溢出，输入应自动提升为胶囊上方附着的 multiline text view；不要把多行内容塞进胶囊里假装可编辑，也不要要求用户先点展开按钮才能看清正在编辑的位置。用户可以主动收起长 draft 以查看上方 transcript；收起态是不可编辑的紧凑摘要和明确的展开 affordance，点击摘要、展开按钮或聚焦 composer 会回到 multiline text view。胶囊本身保持高度不变，展开 editor 内部滚动，并只在确实有更多内容时显示轻微上下渐隐。
